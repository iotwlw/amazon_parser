...
def filter_listing():
    with mysql() as cursor:
        try:
            row_count = cursor.execute("select DISTINCT asin from listing_google_us where state = 0 and review_num >= 100 and review_value >= 4 and asin not in (SELECT asin from merge_product_detail) and asin not in (SELECT asin from merge_product_detail_bak) limit 12")
            ...
            for row in cursor.fetchall():
                    asin = row["asin"]
                    inxxxx = inxxxx + 1
                    print (str(inxxxx)+':'+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))+':'+asin)
